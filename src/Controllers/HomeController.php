...
use AfricaGates\Services\{CacheService,ProfileService,AwardService,StatsService,GlobeBand,RuleEngine,JudgeRubric};

class HomeController {
    public function __construct(private readonly Twig $view,private readonly CacheService $cache,private readonly ProfileService $profiles,private readonly AwardService $awards,private readonly ?StatsService $stats = null){}
    public function index(Request $req,Response $res):Response {
        ...
        return $this->view->render($res,'pages/home.twig',['globe_countries'=>$band['countries'],'globe_note'=>$band['note'],'community_pct'=>(int)round($weights['community']*100),'judge_pct'=>(int)round($weights['judge']*100),'jury_criteria'=>count(JudgeRubric::effective(null)),'page_title'=>'Africa GATES — Continental Cultural Recognition | Afrovanguard','meta_description'=>'Africa GATES is a continental cultural recognition engine — recognising African excellence, live in Nigeria and building across the continent.','gates_page'=>'home','site_stats'=>$this->stats?->summary() ?? $stats,'awards_data'=>$this->cache->remember('home:awards',1800,fn()=>$this->awards->getActiveProgrammesWithStatus()),'leaderboard'=>$this->cache->remember('home:lb',3600,fn()=>$this->profiles->getLeaderboard(8),['leaderboard']),'spotlight_profiles'=>$this->cache->remember('home:spotlight',3600,fn()=>$this->profiles->getFeaturedProfiles(5),['leaderboard']),'latest_winners'=>$this->cache->remember('home:winners',1800,fn()=>\AfricaGates\Services\PublicResults::index(1)['editions'][0] ?? null,['leaderboard']),'site_events'=>$this->cache->remember('home:site_events',900,fn()=>DB::table('gates_site_events')->where('status','published')->where('event_date','>=',date('Y-m-d H:i:s'))->orderBy('event_date')->limit(3)->get()->map(fn($r)=>(array)$r)->all())]);
    }
}
