...
class NominationController {
    ...
    public function form(Request $req,Response $res):Response {
        ...
        return $this->view->render($res,'pages/nominate.twig',['page_title'=>'Nominate — Africa GATES','meta_description'=>'Nominate exceptional African talent for the Africa GATES awards. Put forward the creatives, businesses and changemakers who deserve continental recognition.','gates_page'=>'nominate','programmes'=>$open,'all_programmes'=>$progs,'prefill'=>$prefill,'share_expired'=>$shareToken!=='' && $prefill===null,'regions'=>\AfricaGates\Support\Regions::MAP,'member'=>\AfricaGates\Services\UserAccountService::memberForForms()]);
    }
    public function submit(Request $req,Response $res):Response {
        ...
        $progs=$this->cache->remember('awards:active',1800,fn()=>$this->awards->getActiveProgrammesWithStatus());
        $open=self::openForNominations($progs);
        $rerender=fn(string $msg,int $status=422)=>$this->view->render($res,'pages/nominate.twig',['error'=>$msg,'old'=>$b,'gates_page'=>'nominate','programmes'=>$open,'all_programmes'=>$progs,'regions'=>\AfricaGates\Support\Regions::MAP,'member'=>\AfricaGates\Services\UserAccountService::memberForForms()])->withStatus($status);
