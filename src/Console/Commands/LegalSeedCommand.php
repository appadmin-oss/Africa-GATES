...
/**
 * Write the Terms of Participation and the Privacy Policy into `gates_legal_docs`.
 *
 * ── WHY A COMMAND AND NOT A MIGRATION ────────────────────────────────────────
 *
 * Migrations in this project are schema only — every file under
 * `database/migrations/` creates or alters a table and none of them writes
 * content. A legal document is not schema: it is editorial, it is meant to be
 * edited in `/admin/legal` afterwards, and a migration that rewrote a policy
 * during a routine `db:migrate` would be a very unpleasant surprise.
 *
 * So this is explicit and opt-in. Run it once to install the documents; edit them
 * in admin from then on.
 *
 * ── IT WILL NOT OVERWRITE YOUR EDITS ─────────────────────────────────────────
 *
 * A document that already exists is SKIPPED unless `--force` is passed. That is
 * the important behaviour: the whole point of these living in the database is that
 * an administrator can revise them, and a seeder that clobbered a revised policy
 * every time somebody ran it would silently undo legal review. `--force` says
 * "yes, replace what is there", and the command prints what it replaced.
 *
 ...
 * operator can change per programme and per cycle, and this content is STATIC
 * HTML in a database column — there is no token resolution on the way out, so a
 * figure typed here would be frozen at the moment it was written while the engine
 * moved on. A stale figure in the terms is materially worse than one on a
 * marketing page, because the terms are what a disputed result gets argued
 * against.
 *
 ...
 *
 * This is a faithful, plain-language description of what the platform actually
 * does, written from the code. It is NOT legal advice and it has not been through
 * counsel. Nigerian NDPA specifics, consumer-protection wording and the
 * limitation-of-liability language in particular want a lawyer's eye before this
 * is relied on.
 */
#[AsCommand(
    name: 'legal:seed',
...
final class LegalSeedCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($docs as $slug => $doc) {
            $existing = LegalService::find($slug);

            if ($existing !== null && !$force) {
                $io->writeln("  = <comment>{$slug}</comment> already exists — kept "
            ...
            try {
                ...
                // Loud, not swallowed: a legal document that failed to install is not
                // something to discover from a blank page months later.
                $io->error("  ! {$slug} could not be written: " . $e->getMessage());
                return Command::FAILURE;
            }

            $io->writeln('  + <info>' . $slug . '</info> '
                . ($existing !== null ? 'replaced' : 'installed')
                . ' — ' . number_format(strlen($doc['body'])) . ' bytes, '
                . substr_count($doc['body'], '<h2') . ' sections');
            $wrote++;
        }

        $io->newLine();
        $io->writeln(sprintf('%d written, %d kept.', $wrote, $kept));
        $io->note([
            'Edit these at /admin/legal from now on — this command will not touch them again without --force.',
            'The documents state no percentages on purpose: they point at /integrity, which reads the live',
            'weights from RuleEngine. Do not paste figures into them.',
