...
1. **Data map.** Per capability: what leaves the platform, whose data it is, to which processor, in which jurisdiction, retained how long. Nothing ships without its row.
2. **The nominee-consent problem.** A nominee never submitted anything — a third party nominated them — and their name, organisation, location and a free-text case about them go to a US-hosted LLM. That needs an explicit lawful basis under NDPA 2023, or the data must be minimised until it doesn't. **Recommendation: minimise.** Triage does not need the nominee's identity to score whether a *case* is specific and credible. Redact names/orgs/locations before the prompt, send the reason text with placeholders. Cheaper than a DPA and strictly better for the subject.
3. **Disclosure.** `privacy.twig` gains a real "Automated processing & AI" section: which features use AI, which processors, what data, what it is used for, what it never decides alone, and how to object. Also state it at the point of collection on the nominate form.
4. **Processor terms.** Confirm zero-retention / no-training terms with each provider actually configured, and record which ones are approved for which `pii_class`. Groq's free tier in particular needs checking before it stays the default for a `third_party_subject` capability.
5. **Subject rights.** `privacy:erase-user` already exists — extend it to `gates_ai_calls`/`gates_ai_decisions`, and give nominees a documented objection route.
...
| **D6** | Nominee PII in AI triage (A4): minimise (redact identity) or pursue a lawful basis + DPAs? | **Minimise.** Triage scores whether a *case* is specific and credible; it does not need the nominee's name. Cheaper, faster, strictly better for the subject. |
