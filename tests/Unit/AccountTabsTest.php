...
final class AccountTabsTest extends TestCase
{
    ...
    /**
     * The controller redirects to `/account#referral` after minting a code and after a
     * payout request. That hash must be a tab, or both land on Overview and the member is
     * told nothing happened.
     */
    public function test_the_controllers_redirect_hash_is_a_real_tab(): void
    {
        ...
        foreach (array_unique($m[1]) as $hash) {
            $this->assertContains($hash, self::tabs(),
                "the controller redirects to #{$hash}, which is not a tab — it will land on Overview");
        }
    }
}
